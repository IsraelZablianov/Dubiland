...
- For concept-pipeline coordinators (ideation/spec workstreams), do not create implementation children until a Gate 1 approved spec artifact exists (spec file path + approval comment). Keep the coordinator `blocked` with explicit dependencies to avoid meta-task sprawl and duplicate execution trees.
