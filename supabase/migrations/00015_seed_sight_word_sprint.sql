...
inserted_game AS (
  ...
  SELECT
    ...
    'games.sightWordSprint.subtitle',
    'sight_word_fluency',
    'SightWordSprintGame',
    3,
    4,
    NULL,
    '/audio/he/games/sight-word-sprint/instructions/intro.mp3',
    true
  FROM topic, age_group
  WHERE NOT EXISTS (
    SELECT 1
    FROM games
    WHERE slug = 'sightWordSprint'
  )
  RETURNING id
),
resolved_game AS (
...
SELECT
  ...
  resolved_game.id,
  1,
  '{"adaptive": true, "rounds": 8, "timedStepUnlock": true}'::jsonb,
  1
FROM resolved_game
WHERE NOT EXISTS (
  SELECT 1
