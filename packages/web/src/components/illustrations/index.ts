export { MascotIllustration } from './MascotIllustration';
export type { MascotVariant } from './MascotIllustration';

export { TopicIllustration } from './TopicIllustration';
export type { TopicIllustrationSlug } from './TopicIllustration';

export { FeatureIllustration } from './FeatureIllustration';
export type { FeatureIllustrationKind } from './FeatureIllustration';
