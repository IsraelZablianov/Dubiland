...
inserted_game AS (
  ...
  SELECT
    ...
    'games.rootFamilyStickers.subtitle',
    'sort_build_read',
    'RootFamilyStickersGame',
    4,
    5,
    NULL,
    '/audio/he/games/root-family-stickers/instructions/intro.mp3',
    true
  FROM topic, age_group
  WHERE NOT EXISTS (
    SELECT 1
    FROM games
    WHERE slug = 'rootFamilyStickers'
  )
  RETURNING id
),
resolved_game AS (
...
SELECT
  ...
  resolved_game.id,
  1,
  '{"adaptive": true, "rounds": 7, "supportsSlowMode": true, "decoyCapPercent": 17}'::jsonb,
  1
FROM resolved_game
WHERE NOT EXISTS (
  SELECT 1
...
WHERE g.slug = 'rootFamilyStickers'
